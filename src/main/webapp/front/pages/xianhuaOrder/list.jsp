...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的鲜花订单 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="modal-dialog" style="z-index: 3000">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <textarea id="xianhuaCommentbackContent" style="width: 500px;height: 300px"></textarea>
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" @click="submitXianhuaCommentback()" class="btn btn-theme">评价</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    <li class="layui-this" @click="pageList(0)">全部鲜花订单</li>
                    <li v-for="item in xianhuaOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                </ul>
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.xianhuaName}}</td>
                            <td style="width: 250px;">
                                <img :src="item.xianhuaPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 80px;">{{item.xianhuaValue}}</td>
                            <td style="width: 60px;">{{item.xianhuaKucunNumber}}</td>
                            <td style="width: 60px;">{{item.xianhuaPrice}}</td>
                            <td style="width: 100px;color: red">{{item.xianhuaOldMoney}}</td>
                            <td style="width: 100px;color: red">{{item.xianhuaNewMoney}}</td>
                            <td style="width: 60px;">{{item.xianhuaClicknum}}</td>
                            <td style="width: 80px;">{{item.shangxiaValue}}</td>
                            <td style="width: 150px;">{{item.xianhuaOrderUuidNumber}}</td>
                            <td style="width: 150px;">{{item.buyNumber}}</td>
                            <td style="width: 150px;">{{item.xianhuaOrderTruePrice}}</td>
                            <td style="width: 80px;">{{item.xianhuaOrderValue}}</td>
                            <td style="width: 80px;">{{item.xianhuaOrderPaymentValue}}</td>
                            <td style="width: 150px;">{{item.xianhuaOrderKuaididanhao}}</td>
                            ...
                            <td style="width: 100px;">
                                ...
                                <button v-if="item.xianhuaOrderTypes==3" @click="refund(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                    <i class="layui-icon">&#xe65e;</i> 退款
                                </button>


                                <button v-if="item.xianhuaOrderTypes==4" @click="receiving(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                ...
                                <button v-if="item.xianhuaOrderTypes==5" @click="commentback(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius btn-theme">
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            xianhuaOrderTypesList : [],
            xianhuaOrderPaymentTypesList : [],
            xianhuaOrderTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            //列表查询
            ,pageList(xianhuaOrderTypes) {
                this.xianhuaOrderTypes = xianhuaOrderTypes;
                // 获取列表数据
                layui.http.request('xianhuaOrder/page', 'get', {
                    page: 1,
                    limit: this.limit,
                    xianhuaOrderTypes: (vue.xianhuaOrderTypes == 0)?null:vue.xianhuaOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('xianhuaOrder/page', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    xianhuaOrderTypes: (this.xianhuaOrderTypes == 0)?null:this.xianhuaOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`xianhuaOrder/pay?id=`+id, 'GET',{}, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
                            time: 2000,
                            icon: 6
            ...
            ,refund(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`xianhuaOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
            ...
            ,receiving(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`xianhuaOrder/receiving?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功收货', {
                            time: 2000,
                            icon: 6
            ...
            ,submitXianhuaCommentback(){
                var xianhuaCommentbackContent = $("#xianhuaCommentbackContent").val();
                var id = $("#id").val();

                layui.http.request("xianhuaOrder/commentback?id="+id+"&commentbackText="+xianhuaCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        // 查询当前字典表相关
            // 订单类型的查询和初始化
        xianhuaOrderTypesSelect();
            // 支付类型的查询和初始化
        xianhuaOrderPaymentTypesSelect();
    // 当前表的字典表
        // 订单类型的查询
        function xianhuaOrderTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xianhua_order_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.xianhuaOrderTypesList = res.data.list;
                }
            });
        }
        // 支付类型的查询
        function xianhuaOrderPaymentTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xianhua_order_payment_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.xianhuaOrderPaymentTypesList = res.data.list;
                }
            });
        }
        ...
        // 获取列表数据
        http.request('xianhuaOrder/page', 'get',{
            page: 1,
            limit: limit,
            xianhuaOrderTypes: (vue.xianhuaOrderTypes == 0)?null:vue.xianhuaOrderTypes
        }, function(res) {
            ...
            laypage.render({
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('xianhuaOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            xianhuaOrderType: (vue.xianhuaOrderTypes == 0)?null:vue.xianhuaOrderTypes
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
