    ...
    <!-- 标题 -->
            ...
            <div class="common_list_con clearfix">
                ...
                <ul class="goods_list_td clearfix"  v-for="(item,index) in dataList" v-bind:key="index">
                    ...
                    <li class="col02"><img :src="item.xianhuaPhoto"></li><!-- style="width: 100px;height: 100px;object-fit: cover;"-->
                    <li class="col03">{{item.xianhuaName}}</li>
                    <li class="col05">{{item.xianhuaNewMoney}} RMB</li>
                    <li class="col06">{{item.buyNumber}}</li>
					<li class="col07">{{(item.xianhuaNewMoney*item.buyNumber).toFixed(2)}} RMB</li>
                </ul>
            </div>

			<h3 class="common_title">结算</h3>
			<div class="common_list_con clearfix">
				<div class="settle_con" v-if="this.xianhuaOrderPaymentTypes == 1">
				...
				<div class="settle_con" v-if="this.xianhuaOrderPaymentTypes == 2">
		...
		<script>
				...
				computed: {
					totalMoney: function() {
						...
						for (var item of this.dataList) {
							total += item.xianhuaNewMoney * item.buyNumber
						}
						return total;
					}
				},
				methods: {
					...
					,payClick() {
						...
						let data={
                            ...
                            xianhuas:localStorage.getItem('xianhuas'),
                            yonghuId: localStorage.getItem('userid'),
                            xianhuaOrderPaymentTypes: vue.xianhuaOrderPaymentTypes,
						}
                        // 获取商品详情信息
                        layui.http.request(`xianhuaOrder/order`, 'POST', data, (res) => {
                            // 订单编号
                            var orderId = genTradeNo();
							if(res.code == 0){
                                alert("下单成功，点击确定后跳转 我的订单页面");
                                window.location.href='../xianhuaOrder/list.jsp';
							}
                       		 window.location.href='../xianhuaOrder/list.jsp';
					    });
					}
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function() {
                ...
                // 获取鲜花购买的清单列表
				var xianhuas = localStorage.getItem('xianhuas');
				// 转换成json类型，localstorage保存的是string数据
				vue.dataList = JSON.parse(xianhuas);
