...
<body>
    <div class="modal fade" id="plusOrReduceXianhuaKucunNumberModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <%-- 模态框标题--%>
                <div class="modal-header">
                    <h5 class="modal-title" id="plusOrReduceXianhuaKucunNumberTitle">增加或减少库存</h5>
                ...
                <div class="modal-body">
                    数量:<input type="number" id="plusOrReduceXianhuaKucunNumber" style="width: 300px;" class="form-control form-control-sm"
                    ...
                    <input type="hidden" id="xianhuaId">
                    <input type="hidden" id="hideflag">
                    <input type="hidden" id="thisXianhuaKucunNumber">
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" onclick="submitPlusOrReduceXianhuaKucunNumber()" class="btn btn-primary">提交</button>
                </div>
            </div>
        </div>
    </div>
...
<!--/Pre Loader -->
                ...
                <div class="row breadcrumb-bar">
                    ...
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">鲜花列表</li>
                        </ol>
                    </div>
                </div>
            </div>
    ...
    <!-- 本表的查询条件 -->
        ...
        function setDataRow(item, number) {
            ...
            //鲜花库存
            var xianhuaKucunNumberCell = document.createElement('td');
            xianhuaKucunNumberCell.innerHTML = item.xianhuaKucunNumber;
            row.appendChild(xianhuaKucunNumberCell);


            //购买获得积分
            var xianhuaPriceCell = document.createElement('td');
            xianhuaPriceCell.innerHTML = item.xianhuaPrice;
            row.appendChild(xianhuaPriceCell);


            //鲜花原价
            var xianhuaOldMoneyCell = document.createElement('td');
            xianhuaOldMoneyCell.innerHTML = item.xianhuaOldMoney;
            row.appendChild(xianhuaOldMoneyCell);


            //现价/积分
            var xianhuaNewMoneyCell = document.createElement('td');
            xianhuaNewMoneyCell.innerHTML = item.xianhuaNewMoney;
            row.appendChild(xianhuaNewMoneyCell);


            //点击次数
            var xianhuaClicknumCell = document.createElement('td');
            xianhuaClicknumCell.innerHTML = item.xianhuaClicknum;
            row.appendChild(xianhuaClicknumCell);
            ...
            var plusAttr = "plusXianhuaKucunNumber(" + item.id+','+item.xianhuaKucunNumber+")";
            ...
            var reduceAttr = "reduceXianhuaKucunNumber(" + item.id+','+item.xianhuaKucunNumber+')';
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("xianhua/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addxianhua", "addxianhua");
            window.location.href = "add-or-update.jsp"
        }
        ...
        function shangxia(id,shangxiaTypes) {
            ...
            httpJson("xianhua/update", "POST", data, (res) => {
                if(res.code == 0){
                    alert(msg);
                    getDataList()
                }else{
    ...
    //查询当前表搜索条件所有列表
        ...
        //增加库存
        function plusXianhuaKucunNumber(thisId,xianhuaKucunNumber) {
            if(thisId == null || thisId=="" || thisId=="null"){
                alert("未知错误,请联系管理员处理");
                return;
            }
            $("#plusOrReduceXianhuaKucunNumberTitle").val("增加库存");
            $("#thisXianhuaKucunNumber").val(xianhuaKucunNumber);
            $("#hideflag").val("+");
            $("#xianhuaId").val(thisId);
            id=thisId;
            $('#plusOrReduceXianhuaKucunNumberModal').modal('show');
        }

        //减少库存
        function reduceXianhuaKucunNumber(thisId,xianhuaKucunNumber) {
            if(thisId == null || thisId=="" || thisId=="null"){
                alert("未知错误,请联系管理员处理");
                return;
            }
            $("#plusOrReduceXianhuaKucunNumberTitle").val("减少库存");
            $("#thisXianhuaKucunNumber").val(xianhuaKucunNumber);
            $("#hideflag").val("-");
            $("#xianhuaId").val(thisId);
            id=thisId;
        ...
        $('#plusOrReduceXianhuaKucunNumberModal').on('show.bs.modal', function () {
            //清理表格内容,防止重复回显
            $("#plusOrReduceXianhuaKucunNumber").val(0);
            if(id ==null){
                alert("获取当前列id错误,请联系管理员处理");
                $('#plusOrReduceXianhuaKucunNumberModal').modal('hide');
                $("#plusOrReduceXianhuaKucunNumberTitle").val("");
                $("#thisXianhuaKucunNumber").val("");
                $("#hideflag").val("");
                $("#xianhuaId").val("");
                getDataList();
                return;
        ...
        //模态框关闭的时候会执行这个里面的东西
        $('#plusOrReduceXianhuaKucunNumberModal').on('hide.bs.modal', function () {
            id=null;
            $("#plusOrReduceXianhuaKucunNumber").val(0);
            $("#plusOrReduceXianhuaKucunNumberTitle").val("");
            $("#thisXianhuaKucunNumber").val("");
        ...
        function submitPlusOrReduceXianhuaKucunNumber() {
            var id = $("#xianhuaId").val();//当前数据id
            var hideflag = $("#hideflag").val();//是加还是减
            var thisXianhuaKucunNumber = $("#thisXianhuaKucunNumber").val();//原先的数量
            var plusOrReduceXianhuaKucunNumber = $("#plusOrReduceXianhuaKucunNumber").val();//增加或者减少的数量
            var sumNumber=0;
            var msg;
            if(hideflag == "+"){
                sumNumber =parseInt(thisXianhuaKucunNumber) + parseInt(plusOrReduceXianhuaKucunNumber);
                msg = "增加库存成功";
            }else if(hideflag == "-"){
                sumNumber =parseInt(thisXianhuaKucunNumber) - parseInt(plusOrReduceXianhuaKucunNumber);
                msg = "减少库存成功";
            ...
            let data = {"id":id,"xianhuaKucunNumber":sumNumber};
            httpJson("xianhua/update", "POST", data, (res) => {
                if(res.code == 0){
                    alert(msg);
                    getDataList();
